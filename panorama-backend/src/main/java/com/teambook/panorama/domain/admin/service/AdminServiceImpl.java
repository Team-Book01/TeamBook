...
import com.teambook.panorama.domain.admin.repository.AdminMapper;
...
public class AdminServiceImpl implements AdminService {
  ...
  private final AdminMapper adminMapper;   // 대시보드 전용 집계(MyBatis)
  private final ReportMapper reportMapper; // 신고 대기 목록은 기존 신고 목록 쿼리 재사용

  @Override
  public DashboardResponse getDashboard() {
    return new DashboardResponse(
        adminMapper.selectDashboardStats(),
        selectPendingReports(),
        adminMapper.selectPendingInquiries(PREVIEW_LIMIT),
        adminMapper.selectRecentContents(PREVIEW_LIMIT));
  }
